...
export const TabBar: React.FC = () => {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-30 backdrop-blur-xl bg-black/20 border-b border-white/10 flex items-center gap-1 px-2 py-1 overflow-x-auto">
      {tabs.map((tab) => (
        <motion.div
          key={tab.id}
          layout
          ...
          className={`group relative flex items-center gap-2 px-4 py-2 rounded-t-lg cursor-pointer transition-all ${
            activeTabId === tab.id ? 'bg-white/15' : 'hover:bg-white/5'
          }`}
